...
public class RestDslMainRoute extends RouteBuilder {
    ...
    // @formatter:off
        ...
        .get(env.getProperty("endpoint.getaccounstbydocument")).id("GET_ACCOUNTS_BY_DOCUMENTS").description(env.getProperty("api.description.getaccounstbydocument")).outType(String.class)
		    .param().name("custType").description("Valor de Tipo Documento").dataType("int").defaultValue("false")
			.required(true)
			.endParam()
			.param().name("custPermId").description("Valor documento cliente").dataType("int").defaultValue("false")
			.required(true)
			.endParam()
			.param().name("restrictedDbInd").description("Valor documento cliente").dataType("int").defaultValue("false")
			.required(false)
			.endParam()
			.param().name("restrictedDrInd").description("Valor documento cliente").dataType("int").defaultValue("false")
			.required(false)
			.endParam()
			.param().name("lockStatusInd").description("Valor documento cliente").dataType("int").defaultValue("false")
			.required(false)
			.endParam()
